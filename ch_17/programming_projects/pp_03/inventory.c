...
void erase(void)
{
    ...
    for (cur = inventory, prev = NULL; cur != NULL && cur->number != part_num; prev = cur, cur = cur->next)
        ;
    ...
    if (prev == NULL && cur->number == part_num)
        inventory = cur->next;
    else
        prev->next = cur->next;

    free(cur);
    printf("Part num %d is successfully erased!", part_num);
}
